...
var url = "mongodb://192.168.64.2:27017";
...
io.on('connection', (socket) => {
    ...
    socket.on('host-join-game', (data) => {
        ...
        if(game){
            ...
            MongoClient.connect(url, function(err, db){
                ...
                dbo.collection("kahootAnswers").find(query).toArray(function(err, res) {
                    if (err) throw err;
                    
                    //var question = res[0].questions[0].question;
                    var answer1 = res[0].answer;
                    //var answer2 = res[0].questions[0].answers[1];
                    //var answer3 = res[0].questions[0].answers[2];
                    //var answer4 = res[0].questions[0].answers[3];
                    //var correctAnswer = res[0].questions[0].correct;
                    
                    socket.emit('GameAnswers', {
                        //q1: question,
                        a1: answer1,
                        //a2: answer2,
                        //a3: answer3,
    ...
    socket.on('submitAnswerA', function(data){  
        console.log("answer started");
        MongoClient.connect(url, function(err, db){
            if (err) throw err;
            var dbo = db.db('kahootDB');
            dbo.collection('kahootAnswers').find({}).toArray(function(err, result){
                if(err) throw err;
                ...
                var answer = data;
                dbo.collection("kahootAnswers").insertOne(answer, function(err, res) {
                    if (err) throw err;
                    db.close();
                });
                db.close();
